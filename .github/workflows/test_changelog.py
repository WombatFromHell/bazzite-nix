...
def temp_git_repo():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        initial_hash = subprocess.run(["git", "rev-parse", "HEAD"], cwd=tmpdir, check=True, capture_output=True, text=True).stdout.strip()
        
        # Create second commit
        test_file.write_text("updated")
        subprocess.run(["git", "add", "."], cwd=tmpdir, check=True, capture_output=True)
        subprocess.run(["git", "commit", "-m", "Update test file"], cwd=tmpdir, check=True, capture_output=True)
        final_hash = subprocess.run(["git", "rev-parse", "HEAD"], cwd=tmpdir, check=True, capture_output=True, text=True).stdout.strip()
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.json', delete=False) as f:
            json.dump({
                "variants": [
                    ...
                    {"name": "disabled", "base_image": "test:latest", "suffix": "", "disabled": True},
                ]
            }, f)
            temp_path = f.name
...
class TestGetTagsFromManifests:
    ...
    def test_resolves_tags_from_variant_patterns(self, sample_manifests_curr, sample_variants):
    ...
    def test_handles_canonical_tag_pattern(self, sample_manifests_curr, sample_variants):
        ...
        assert (re.match(canonical_pattern, prev_tag) or 
                re.match(canonical_pattern, curr_tag) or
                prev_tag == "testing" or curr_tag == "testing")
...
class TestGetManifestsForTarget:
    ...
    def test_filters_disabled_variants(self, monkeypatch):
        ...
        manifests = changelog.get_manifests_for_target("testing", variants_with_disabled)
...
class TestGetCommits:
    ...
    def test_extracts_commits_from_git_history(self, temp_git_repo, sample_manifests_prev, sample_manifests_curr):
        ...
        sample_manifests_prev["testing"]["Labels"]["org.opencontainers.image.revision"] = start_hash
        sample_manifests_curr["testing"]["Labels"]["org.opencontainers.image.revision"] = end_hash
    ...
    def test_skips_merge_commits(self, temp_git_repo, sample_manifests_prev, sample_manifests_curr):
        ...
        subprocess.run(["git", "add", "."], cwd=tmpdir, check=True, capture_output=True)
        subprocess.run(["git", "commit", "-m", "Add feature"], cwd=tmpdir, check=True, capture_output=True)
        ...
        subprocess.run(["git", "merge", "--no-ff", "-m", "Merge feature", "feature"], cwd=tmpdir, check=True, capture_output=True)
        merge_hash = subprocess.run(["git", "rev-parse", "HEAD"], cwd=tmpdir, check=True, capture_output=True, text=True).stdout.strip()
        
        sample_manifests_prev["testing"]["Labels"]["org.opencontainers.image.revision"] = start_hash
        sample_manifests_curr["testing"]["Labels"]["org.opencontainers.image.revision"] = merge_hash
    ...
    def test_handles_missing_revision(self, sample_manifests_prev, sample_manifests_curr):
        """Should handle missing revision labels."""
        # Remove revision labels
        del sample_manifests_prev["testing"]["Labels"]["org.opencontainers.image.revision"]
        del sample_manifests_curr["testing"]["Labels"]["org.opencontainers.image.revision"]
    ...
    def test_handles_invalid_workdir(self, sample_manifests_prev, sample_manifests_curr):
        """Should handle invalid working directory."""
        sample_manifests_prev["testing"]["Labels"]["org.opencontainers.image.revision"] = "abc123"
        sample_manifests_curr["testing"]["Labels"]["org.opencontainers.image.revision"] = "def456"
    ...
    def test_includes_commit_links(self, temp_git_repo, sample_manifests_prev, sample_manifests_curr):
...
class TestIntegration:
    ...
    def test_full_changelog_generation(
        self, sample_variants, sample_manifests_prev, sample_manifests_curr, temp_git_repo
    ):
    ...
    def test_main_function_writes_files(self, sample_variants, sample_manifests_curr, variants_config_file):
...
class TestChangelogFormat:
    ...
    def test_changelog_tables_have_correct_structure(
        ...
        assert "| --- | --- | --- | --- |" in changelog_text or "| --- | --- |" in changelog_text
        # Table rows should start and end with pipes
        table_rows = [line for line in changelog_text.split("\n") if line.strip().startswith("|") and "---" not in line]
    ...
    def test_changelog_uses_correct_emoji_indicators(
        ...
        curr = {
            "testing": {
                ...
                "Labels": {
                    "dev.hhd.rechunk.info": json.dumps({
                        "packages": {"added-pkg": "2.0.0", "changed-pkg": "2.0.0"}  # Version changed from 1.0.0
                    }),
                },
            }
        }
    ...
    def test_changelog_no_unfilled_placeholders(
        ...
        # Common placeholder patterns that should be filled
        assert "{" not in changelog_text or "{changes}" not in changelog_text
        # All template variables should be replaced
        for placeholder in ["{handwritten}", "{target}", "{prev}", "{curr}", "{changes}", "{pkgrel:"]:
            assert placeholder not in changelog_text, f"Unfilled placeholder: {placeholder}"
...
class TestConfigurationLoading:
    ...
    def test_registry_from_environment(self):
        """Should use registry from environment."""
        original = os.environ.get("REGISTRY", "")
        
        try:
            os.environ["REGISTRY"] = "ghcr.io/test-owner"
            # Reload module to pick up new env
            import importlib
            importlib.reload(changelog)
            
            assert changelog.REGISTRY_BASE == "ghcr.io/test-owner"
        finally:
            if original:
                os.environ["REGISTRY"] = original
            else:
    ...
    def test_repo_owner_lowercase(self):
        """Should ensure repo owner is lowercase."""
        original = os.environ.get("GITHUB_REPOSITORY_OWNER", "")
        ...
        try:
            ...
            if original:
                os.environ["GITHUB_REPOSITORY_OWNER"] = original
            else:
